...
function removeFirstOccurrences(str, value) {
  ...
  return str.slice(0, num).concat(str.slice(num + value.length, str.length));
}
...
function formatTime(minutes, seconds) {
  const fMin = minutes.toString();
  const fSec = seconds.toString();
  return fMin.padStart(2, '0').concat(':', fSec.padStart(2, '0'));
}
...
function findLongestWord(sentence) {
  let longWord = '';
  const sentWords = sentence.split(' ');
  for (let i = 0; i < sentWords.length; i += 1) {
    if (sentWords[i].length > longWord.length) {
      longWord = sentWords[i];
    }
  }
  return longWord;
}
...
function reverseWords(str) {
  ...
  for (let i = 0; i < revStr.length; i += 1) {
    revWord = '';
    for (let y = revStr[i].length - 1; y >= 0; y -= 1) {
      revWord = revWord.concat(revStr[i].charAt(y));
    }
    revStr[i] = revWord;
  }
  revStr = revStr.join(' ');
...
function invertCase(str) {
  let invCaseStr = str.split('');
  for (let i = 0; i < str.length; i += 1) {
    if (invCaseStr[i] === invCaseStr[i].toUpperCase()) {
      invCaseStr[i] = invCaseStr[i].toLowerCase();
    } else {
      invCaseStr[i] = invCaseStr[i].toUpperCase();
    }
  }
  invCaseStr = invCaseStr.join('');
  return invCaseStr;
}
...
function encodeToRot13(str) {
  let fCoded = str.split('');
  const source = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const krypt = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  const sourceArr = source.split('');
  const kryptArr = krypt.split('');
  let fIndex = 0;
  for (let i = 0; i < str.length; i += 1) {
    if (sourceArr.indexOf(fCoded[i]) >= 0) {
      fIndex = sourceArr.indexOf(fCoded[i]);
      fCoded[i] = kryptArr[fIndex];
    }
  }
  fCoded = fCoded.join('');
  return fCoded;
